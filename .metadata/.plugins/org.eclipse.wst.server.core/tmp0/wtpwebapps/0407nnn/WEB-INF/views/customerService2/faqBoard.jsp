	...
	<div id="wrap">
		<section class="faq-view">
			...
			<c:if test="${list.count == 0}">
					<div class="faq-table-count-zero">
						게시판에 저장된 글이 없습니다.
					</div>
			</c:if>
			<c:if test="${list.count > 0}">
			...
			<c:if test="${list.count > 0}">
				<div id="faq-paging-wrap">
					<c:if test="${list.p.beginPageNumber > 10}">
						<a href="faqViewCategory?p=${list.p.beginPageNumber-1}">이전</a>
					</c:if>
					<c:forEach var="pno" begin="${list.p.beginPageNumber}"
						end="${list.p.endPageNumber}">
						<a href="faqViewCategory?category=${category}&p=${pno}">[${pno}]</a>
					</c:forEach>
					<c:if test="${list.p.endPageNumber < list.p.totalPageCount}">
						<a href="faqViewCategory?p=${list.p.endPageNumber + 1}">다음</a>
					</c:if>
				</div>
			</c:if>
		</section>
					...
					.done(
							function(result) {
								...
								if (list.beginPageNumber > 10) {
									pazing += "<a href=" + "'category=" + result.category + "&p="
								...
								for (let pno = list.beginPageNumber; pno <= list.endPageNumber; pno++) {
									...
									pazing += "<a href='faqViewCategory?category=" + result.category + "&p=" + pno + "'>["
											+ pno + "]</a>";
								}

								if (list.endPageNumber < list.totalPageCount) {
									pazing += "<a href='faqViewCategory?category="+ result.category +"&p="
