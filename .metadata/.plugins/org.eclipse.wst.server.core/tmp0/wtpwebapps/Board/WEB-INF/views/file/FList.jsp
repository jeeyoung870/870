...
<body>
<center>
<b>자료 목록(전체 글:${list.count})</b>
<table width="700">
  <tr>
    <td align="right" >
       <a href="fWriteForm">업로드</a>
    </td>
  </tr>
</table>
...
<table border="1" width="700" cellpadding="0" cellspacing="0" align="center">
  ...
  <c:if test="${article.re_level == 0}">
    <img src="resources/images/level.gif" width="${5 * article.re_level}" height="16">
      ...
      <a href="fContent?num=${article.num}&p=${list.requestPage}">
          ${article.subject}</a>
...
<c:if test="${list.count > 0}">
  <c:if test="${list.p.beginPageNumber > 10}">
			<a href="flist?p=${list.p.beginPageNumber-1}&per=${per}">이전</a>
		</c:if>
		<c:forEach var="pno" begin="${list.p.beginPageNumber}" end="${list.p.endPageNumber}">
		<a href="flist?p=${pno}&per=${per}">[${pno}]</a>
		</c:forEach>
		<c:if test="${list.p.endPageNumber < list.p.totalPageCount}">
			<a href="flist?p=${list.p.endPageNumber + 1}&per=${per}">다음</a>
		</c:if>
</c:if>
</center>
