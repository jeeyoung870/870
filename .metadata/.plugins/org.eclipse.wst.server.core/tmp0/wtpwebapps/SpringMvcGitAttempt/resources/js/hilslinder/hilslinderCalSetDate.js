			...
			for (let weekindex = 0; weekindex < 6 ; weekindex++){
				for(let weekdayindex = 0 ; weekdayindex < 7 ; weekdayindex++){
					...
					if(weekindex==0 && weekdayindex < firstDayName){
						if (weekdayindex == 0){
							html += "<div class = 'calendar_day prevMonth'><span>" + (lastDayOfPrevMonth - (firstDayName - 1) + weekdayindex) + "</span><span></span></div>"
						...
						if(weekdayindex == 0){
							html += "<div class = 'calendar_day thisMonth'><span>" + startDayCount + "</span><span id='" + year + month + setFixDayCount(startDayCount++) + "' class = 'thisMonthDate'></span></div>";
						}else if(weekdayindex == 6){
							html += "<div class = 'calendar_day thisMonth'><span>" + startDayCount + "</span><span id='" + year + month + setFixDayCount(startDayCount++) + "' class = 'thisMonthDate'></span></div>";
						}else{
							html += "<div class = 'calendar_day thisMonth'><span>" + startDayCount + "</span><span id='" + year + month + setFixDayCount(startDayCount++) + "' class = 'thisMonthDate'></span></div>";
