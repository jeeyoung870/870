...
<style type="text/css">
    ...
    #profile, .p123, .innerdiv { width: 380px; }
    #profile {display:block; padding-bottom: 40px; }
    .p123{display:block; padding-top:30px; }
...
<center>
  ...
  <form action="savePInfo" name="userProfile" method="POST" onsubmit="return profileSubmit()">
	  ...
	  <div class="p123">
	  	키 (cm)<br>
	  	<input type="number" name="height" id="height" value="${pInfo.height }"/><br><br>
	  	체중 (kg)<br>
	  	<input type="number" name="weight" id="weight" value="${pInfo.weight }" /><br><br>
	  	한줄 소개<br>
	  	<textarea cols="40" rows="5" name="introduce" id="introduce">${pInfo.introduce }</textarea>
	  </div>
