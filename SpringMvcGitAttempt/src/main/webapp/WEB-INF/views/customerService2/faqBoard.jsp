			...
			$.ajax({
					...
					.done(
							function(result) {
								...
								let htmls = "";
								let pazing = "";
								htmls += "<tr><td>게시글번호</td><td>제목</td><td>작성자</td></tr>"
								result.faqList.forEach(function(elem) {
									htmls += "<tr><td>" + elem.num + "</td>"
									htmls += "<td><a href='./faqViewContent?num="
											+ elem.num + "&p="+ result.requestPage +"'>" + elem.subject
											+ "</a></td>"
									htmls += "<td>" + "관리자" + "</td></tr>"
								})
								...
								if(list.beginPageNumber > 10){
									pazing += "<a href=" + "'faqboard?p=" + list.beginPageNumber-1 + "'>이전</a>";
								}
								
								for(let pno = list.beginPageNumber ; pno <= list.endPageNumber ; pno++){
									
									pazing += "<a href='faqboard?p=${pno}'>[" + pno + "]</a>";
								}
								
								if(list.endPageNumber < list.totalPageCount){
									pazing += "<a href='faqboard?p="+list.endPageNumber + 1+"'>다음</a>"
								}
